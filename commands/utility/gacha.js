...
const ampData = require("../../ampData.json");
...
function pickRandomCard(cardPool) {
  if (!cardPool || cardPool.length === 0) {
    return {card: null, remainingPool: []};
  }
  ...
  return {card: selectedCard, remainingPool};
}
  ...
  data: new SlashCommandBuilder()
    ...
    if (userInfo.rowNumber > 0) {
      ...
      try {
        ...
        let mainCardPool = ampData.filter(card => card.category !== "Trap Card");
        for (let i = 0; i < 727; i++) {
          mainCardPool = shuffleArray(mainCardPool);
        }
        const shuffledTrapCards = shuffleArray(trapCards);
        for (let i = 0; i < 3; i++) {
          const result = pickRandomCard(mainCardPool);
          if (result.card) {
