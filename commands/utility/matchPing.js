...
const {parseSheetDateTimeToUnix} = require("../../modules/dateTimeHelpers.js");
...
const qualifiersCaptainStart = sheetsConfig.qualifiersCaptainDiscordStartingColumn;
...
async function sendMatchPingWithMedia(client, content, embed, mediaLink, label) {
    ...
    try {
        ...
        try {
            await channel.send({ content: content + "\n*(Note: Media failed to load)*", embeds: [embed] });
        } catch (fallbackError) {
...
function qualifiers_ping(client) {
    ...
    return getSpreadsheetData(qualifiersSheet).then(rows => {
        const lobbiesPinged = [];
        const updatePromises = [];
        ...
        rows.forEach((row, rowIndex) => {
            ...
            const alreadyPinged = row[columnToIndex(qualifiersPingCheck)] === "TRUE";
            if (alreadyPinged) return;

            const lobbyTimeUnix = parseSheetDateTimeToUnix(dateStr, timeStr, config.timezone);
            ...
            if (timeDifference <= 15 && timeDifference > 0) {
                ...
                const mediaLink = getRandomMedia();
                const captainFieldValue = captainIds.length > 0
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields(
                        {name: "Lobby Time", value: `<t:${Math.floor(lobbyTimeUnix)}:f>`, inline: false},
                        {name: `Captains (${captainIds.length})`, value: captainFieldValue, inline: false}
                    )
                    .setTimestamp()
                    .setFooter({text: `Lobby starting in ${Math.round(timeDifference)} minutes.`});
...
async function force_qualifiers_ping(client, lobbyId) {
    try {
        ...
        for (let i = 0; i < rows.length; i++) {
            const row = rows[i];
            if (row[columnToIndex(qualifiersLobbyIdColumn)]?.toUpperCase() !== lobbyId.toUpperCase()) continue;
...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName("check")
                .setDescription("Check for matches/lobbies that are 15 minutes away or less")
        )
        .addSubcommand(subcommand =>
            subcommand
                .setName("force")
                .setDescription("Force send a ping for a specific match or qualifier lobby")
    ...
    execute: async (interaction) => {
        ...
        if (subcommand === "check") {
            await interaction.reply({content: "Checking for matches and qualifier lobbies...", ephemeral: true});
            ...
            if (total > 0) {
                const parts = [];
                if (bracketPinged.length > 0) parts.push(`Bracket: ${bracketPinged.join(", ")}`);
                if (qualifiersPinged.length > 0) parts.push(`Qualifiers: ${qualifiersPinged.join(", ")}`);
                ...
                await interaction.editReply({
                    content: "Check completed! No matches or lobbies found that need pinging.",
                    ephemeral: true
                });
            }
